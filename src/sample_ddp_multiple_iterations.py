...
def main(args):
    ...
    ckpt_root = model_config.get("ckpt")
    model_config["ckpt"] = f"{ckpt_root}/ep-0000000.pt"
    ...
    if args.clamp_pred:
        sampler_params["clamp_pred"] = args.clamp
    ...
    while True:
        ...
        current_epoch += args.epoch_frequency
        ckpt_path = f"{ckpt_root}/ep-0000{current_epoch:03d}.pt"    
        ckpt_string_name = "pretrained" if not ckpt_path else os.path.splitext(os.path.basename(str(ckpt_path)))[0]

        if not os.path.exists(ckpt_path):
            print(f"WARNING: NOT FOUND CKPT {ckpt_path} for epoch {current_epoch}, exit")
            break  
        ...
        sample_folder_dir = os.path.join(sample_folder_dir, f"Epoch_{current_epoch}")
        if rank == 0:
        ...
        for step_idx in pbar:
            with autocast(**autocast_kwargs):
                ...
                if args.clamp > 0:
                    samples = samples.clamp(min=-args.clamp, max=args.clamp)
                    print("clamping")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epoch-frequency", type=int, default=40)
    ...
    parser.add_argument("--clamp_pred", type=int, default=0)
