...
class SigLIP2wNorm(nn.Module):
    ...
    def forward(self, images):
        ...
        latents = self.model.embeddings(images, interpolate_pos_encoding = True)
        for layer in self.model.encoder.layers:
            latents = layer(latents, None)[0]
        image_features = self.model.post_layernorm(latents)

        # # checking if output is exactly the same
        # outputs = self.model(images, output_hidden_states=True, interpolate_pos_encoding = True)
        # image_features_ref = outputs.last_hidden_state
        # diff = (image_features_ref - image_features).abs()
        # print("max abs diff:", diff.max().item(), "mean abs diff:", diff.mean().item())
        # raise ValueError("Debugging")
        return image_features, latents
