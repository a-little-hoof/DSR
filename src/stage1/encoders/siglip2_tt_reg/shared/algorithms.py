...
def find_register_neurons(
  ...
  for i in tqdm(range(len(random_images)), desc="Processing random images"):
    ...
    if neurons_to_ablate_input:
      hook_manager.intervene_register_neurons(neurons_to_ablate=neurons_to_ablate_input, num_registers = num_registers, normal_values=NORMAL_VALUES, scale = 1)
    hook_manager.finalize()

    representation = run_model(model, image)
    baseline_neuron_acts = hook_manager.get_neuron_activations()
    baseline_layer_outputs = hook_manager.get_layer_outputs()
    try:
      baseline_neuron_acts = torch.from_numpy(baseline_neuron_acts).to(device)
      baseline_layer_outputs = torch.from_numpy(baseline_layer_outputs).to(device)
    except:
      pass
