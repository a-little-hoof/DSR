...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--debug", action="store_true", help="Set a much smaller batch size for debugging")
    
    parser.add_argument('--save-features-only', action='store_true', help='Only run encoder and save features, then exit.')
    parser.add_argument('--load-features-from', type=str, default=None, help='Directory to load precomputed features from (used with --save-features-only).')
    
    return parser.parse_args()


def prepare_dataloader_from_features(
    features_path: Path,
    batch_size: int,
    workers: int,
    rank: int,
    ...
    class FeatureDataset(torch.utils.data.Dataset):
        def __init__(self, features_dir: Path):
            self.features_files = sorted(glob(f"{features_dir}/*.pt"))

        def __len__(self):
            return len(self.features_files)

        def __getitem__(self, idx):
            data = torch.load(self.features_files[idx])
            return data['image'], data['feature']
...
def main():
    ...
    if args.debug:
        print("DEBUGGING: OVERRIDING BATCHSIZE TO A SMALL VALUE")
        batch_size = 16
    ...
    rae.encoder.eval()
    rae.decoder.train()
    encoder_module = rae.encoder.__class__.__module__
    has_siglip2_tt_reg = "siglip2_tt_reg" in encoder_module
    if has_siglip2_tt_reg:
        ema_model = instantiate_from_config(rae_config).to(device)
        if args.compile:
            ema_model.encode = torch.compile(ema_model.encode)
            ema_model.forward = torch.compile(ema_model.forward)
        ema_model.load_state_dict(rae.state_dict(), strict=True)
        logger.info("Reinitialized ema_model for SigLIP2 TTREG to preserve hook links.")
    else:
        ema_model = deepcopy(rae).to(device)
    ema_model.eval()
    ...
    if args.load_features_from is not None:
        loader, sampler = prepare_dataloader_from_features(
            args.load_features_from, batch_size, num_workers, rank, world_size
        )
    else:
        loader, sampler = prepare_dataloader(
            args.data_path, batch_size, num_workers, rank, world_size, transform=stage1_transform, shuffle=not args.save_features_only
        )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        num_batches = 0
        if checkpoint_interval > 0 and epoch % checkpoint_interval == 0  and rank == 0 and not(args.save_features_only):
        ...
        for step, (images, _) in tqdm(enumerate(loader)):
            ...
            with autocast(**autocast_kwargs):
                # if args.load_features_from is None:
                #   with torch.no_grad():
                #         z = ddp_model.module.encode(images)
                #         if args.save_features_only:
                #             if epoch == 0:
                #                 # save features and images as a dict
                #                 z_cpu = z.cpu()
                #                 # save pair one by one
                #                 for i in range(z_cpu.size(0)):
                #                     pair = {
                #                         'image': images[i].detach().cpu().clone(),
                #                         'feature': z_cpu[i].detach().cpu().clone(),
                #                         'label': _[i].detach().cpu().clone(),
                #                     }
                #                     # print(f"Saving feature-image pair {global_idx}...")
                #                     # print(f"Feature shape: {pair['feature'].shape}, Image shape: {pair['image'].shape}, Label: {pair['label']}")
                #                     save_path = os.path.join(experiment_dir, f"image_feature_pair_{global_idx}.pt")
                #                     torch.save(pair, save_path)
                #                     global_idx += 1
                #                 continue  # skip rest of training loop
                #             else:
                #                 import sys
                #                 sys.exit(0)
                # else:
                #     z = _.to(device)
                # recon = ddp_model.module.decode(z) # keep gradient synced
            ...
            if global_step % sample_every == 0:
                ...
                # logger.info("remove ema generating samples temporarily...")
