...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--outlier_token_norm", type=float,  default=-1.0, help="whether or not to filter outlier within loss.")
    parser.add_argument("--token_norm_shift", action="store_true")
    parser.add_argument("--logit_normal_scheduler", action="store_true")
    parser.add_argument("--eps", type=float,  default=1e-12, help="whether or not to filter outlier within loss.")
...
def main():
    ...
    args = parse_args()
    print(f"args.eps = {args.eps:.3e}  (raw: {args.eps})")
    ...
    if args.debug:
        print("FIXING GLOBAL_BATCHSIZE TO 32 FOR DEBUGGING")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            with torch.no_grad(): # TODO: wrap this in autocast?
                z, token_norm = rae.encode(images, output_token_norm=True)
            optimizer.zero_grad(set_to_none=True)
            model_kwargs = dict(y=labels)
            with autocast(**autocast_kwargs):
                if args.outlier_token_norm > 0:
                    high_thr = args.outlier_token_norm
                    low_thr = 0.0
                    loss_mask = ((token_norm < high_thr) & (token_norm >= low_thr)).bool()
                    if args.debug:
                        print(token_norm.mean(), token_norm.max(), token_norm.min())
                    keep_ratio = loss_mask.float().mean().item()
                    filter_ratio = 1.0 - keep_ratio
                    if args.debug:
                        print(f"With norm filter [{low_thr}, {high_thr}), we filter about {filter_ratio} ratio of tokens")
                ...
                final_loss = loss_dict.get("final_loss", None)
                if args.token_norm_shift:
                    eps = args.eps
                    B = token_norm.shape[0]

                    # per-sample median over all token positions (flatten non-batch dims)
                    token_norm_flat = token_norm.reshape(B, -1)
                    token_median = token_norm_flat.median(dim=1).values.clamp_min(eps)  # [B]

                    # broadcast median back to token shape
                    token_median_b = token_median.view(B, *([1] * (token_norm.dim() - 1)))  # [B,1,...]

                    # alpha' = s_current / s_median  (dimension ratio is alpha'^2)
                    token_scale_alpha = (token_norm / token_median_b).clamp_min(eps)

                    # current_t: shape [B] -> broadcast to token shape
                    current_t = loss_dict["t"]  # [B]
                    t_b = current_t.view(B, *([1] * (token_norm.dim() - 1)))  # [B,1,...]

                    # --- alpha_dim from RAE global shift ---
                    # computed earlier as: time_dist_shift = sqrt(shift_dim / shift_base)
                    alpha_dim = float(time_dist_shift)

                    def w_jac(alpha, t):
                        # w_alpha(t) = alpha / (alpha - (alpha-1)*t)^2
                        denom = (alpha - (alpha - 1.0) * t).clamp_min(eps)
                        return alpha / (denom * denom)

                    # --- correct "extra" weight to emulate an additional token shift on top of RAE ---
                    w_dim = w_jac(alpha_dim, t_b)                                               # w_{a_dim}(t)
                    w_tot = w_jac(alpha_dim * token_scale_alpha, t_b)                           # w_{a_dim*a_i}(t)
                    w = w_tot / (w_dim)                                                   # ratio  \tilde w_i(t)

                    if args.logit_normal_scheduler:
                        def inv_g(alpha, t, eps=1e-12):
                            # g_alpha^{-1}(t) = t / (alpha - (alpha-1) t)
                            t = t.clamp(eps, 1.0 - eps)
                            denom = (alpha - (alpha - 1.0) * t).clamp_min(eps)
                            return (t / denom).clamp(eps, 1.0 - eps)
                        def logitnormal_pdf(u, mu=0.0, sigma=1.0, eps=1e-12):
                            # logit(u) ~ N(mu, sigma^2)
                            u = u.clamp(eps, 1.0 - eps)
                            z = torch.log(u) - torch.log1p(-u)          # logit(u)
                            norm = 1.0 / (sigma * math.sqrt(2.0 * math.pi))
                            return norm * torch.exp(-0.5 * ((z - mu) / sigma) ** 2) / (u * (1.0 - u))
                        def logitnormal_logpdf(u, mu=0.0, sigma=1.0, eps=1e-12):
                            # log p(u) where logit(u) ~ N(mu, sigma^2)
                            u = u.clamp(eps, 1.0 - eps)
                            z = torch.log(u) - torch.log1p(-u)  # logit(u)

                            # log Normal(z; mu, sigma^2)
                            log_norm = -math.log(sigma) - 0.5 * math.log(2.0 * math.pi)
                            log_gauss = log_norm - 0.5 * ((z - mu) / sigma) ** 2

                            # Jacobian: 1/(u(1-u))  ->  -log u - log(1-u)
                            log_jac = -torch.log(u) - torch.log1p(-u)
                            return log_gauss + log_jac

                        u0 = inv_g(alpha_dim, t_b, eps=args.eps)
                        u1 = inv_g(alpha_dim * token_scale_alpha, t_b, eps=args.eps)
                        # q_ratio = logitnormal_pdf(u1, mu=0.0, sigma=1.0, eps=args.eps) / logitnormal_pdf(u0, mu=0.0, sigma=1.0, eps=args.eps)
                        logq = logitnormal_logpdf(u1, mu=0.0, sigma=1.0, eps=eps) - \
                                logitnormal_logpdf(u0, mu=0.0, sigma=1.0, eps=eps)
                        logq = logq.clamp(-10.0, 10.0)
                        q_ratio = torch.exp(logq)

                        if args.debug:
                            # batch-0 flatten
                            alpha0 = token_scale_alpha[0].reshape(-1)
                            w0_jac = (w_tot / w_dim)[0].reshape(-1)
                            q0 = q_ratio[0].reshape(-1)

                            print(
                                f"t[0]={current_t[0].item():.4f}; "
                                f"{(alpha0 >= 2.0).float().mean().item()*100.0:.2f}% tokens have alpha[0]>=2; "
                                f"jac_w0 mean={w0_jac.mean().item():.4f}, min={w0_jac.min().item():.4f}, max={w0_jac.max().item():.4f}; "
                                f"{(q0 >= 2.0).float().mean().item()*100.0:.2f}% tokens have q_ratio[0]>=2; "
                                f"{(q0 <= 0.5).float().mean().item()*100.0:.2f}% tokens have q_ratio[0]<=0.5; "
                                f"q_ratio0 mean={q0.mean().item():.4f}, min={q0.min().item():.4f}, max={q0.max().item():.4f}"
                            )
                            print("jac_w0 first 16:", w0_jac[:16].detach().float().cpu().tolist())
                            print("q_ratio0 first 16:", q0[:16].detach().float().cpu().tolist())
                        w = w * q_ratio
                    # --- (optional) stats ---
                    sqrt2 = 2.0 ** 0.5
                    pct_ge_2x_dim = (token_scale_alpha >= sqrt2).float().mean().item() * 100.0
                    pct_le_half_dim = (token_scale_alpha <= 1.0 / sqrt2).float().mean().item() * 100.0
                   
                    # per-sample (batch 0) stats
                    alpha0 = token_scale_alpha[0].reshape(-1)
                    w0 = w[0].reshape(-1)
                    
                    if args.debug:
                        print(f"Token-norm: {pct_ge_2x_dim:.2f}% >=2×dim (alpha>=sqrt2); {pct_le_half_dim:.2f}% <=0.5×dim.")
                        print(
                            f"t[0]={current_t[0].item():.4f}; "
                            f"{(alpha0 >= 2.0).float().mean().item()*100.0:.2f}% tokens have alpha[0]>=2; "
                            f"{(w0 >= 2.0).float().mean().item()*100.0:.2f}% tokens have w_extra[0]>=2; "
                            f"{(w0 <= 0.5).float().mean().item()*100.0:.2f}% tokens have w_extra[0]<=0.5; "
                            f"w0 mean={w0.mean().item():.4f}, min={w0.min().item():.4f}, max={w0.max().item():.4f}"
                        )

                        # if you *really* want to "print w" without flooding logs, print a small slice
                        # (adjust slice depending on your token layout)
                        print("w_extra[0] first 16:", w0[:16].detach().float().cpu().tolist())
                    # --- clamp + post-normalize ---
                    w_min = getattr(args, "w_min", 0.1)
                    w_max = getattr(args, "w_max", 2.0)

                    w = w.clamp(min=w_min, max=w_max)
                else:
                    w = torch.ones_like(final_loss)
                if args.outlier_token_norm > 0:
                    w = w * loss_mask
                    if args.debug:
                        print("Combine outlier filtering with token_norm_shift.")
                w = w / (w.mean().detach() + args.eps)
                loss = mean_flat(final_loss * w).mean()  
                if not args.token_norm_shift and not(args.outlier_token_norm>0):
                    loss_ref =  loss_dict.get("loss", None).mean()
                    diff = (loss - loss_ref).detach()
                    if args.debug:
                        print(f"[check] loss={loss.item():.8e} loss_ref={loss_ref.item():.8e} diff={diff.item():.8e}")
            loss.float()
            if scaler:
            ...
            if global_step % sample_every == 0:
                ...
                model.train()
            if do_eval and (eval_interval > 0 and global_step % eval_interval == 0) and not(args.debug):
