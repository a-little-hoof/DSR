...
class Transport:
    ...
    def training_losses(
        ...
        if self.model_type == ModelType.VELOCITY:
            final_loss = ((model_output - ut) ** 2)
            # terms['loss'] = mean_flat(((model_output - ut) ** 2))
            terms['loss'] = mean_flat(final_loss)
            terms['final_loss'] = final_loss
            terms['t'] = t
        elif self.model_type == ModelType.DATA:
            # (x_pred - z) / (1 - t).clamp_min(self.t_eps)
            t_ = t.view(-1, 1, 1, 1)   
            # model_output = (model_output - xt)/(1 - t_).clamp_min(self.t_eps)
            model_output = (xt - model_output) / t_.clamp_min(self.t_eps)
            # terms['loss'] = mean_flat(((model_output - ut) ** 2))
            final_loss = ((model_output - ut) ** 2)
            terms['loss'] = mean_flat(final_loss)
            terms['final_loss'] = final_loss
            ...
            if self.model_type == ModelType.NOISE:
                ...
                final_loss = weight * ((model_output - x0) ** 2)
                terms['loss'] = mean_flat(final_loss)
                terms['final_loss'] = final_loss
                terms['t'] = t
                ...
                final_loss = weight * ((model_output * sigma_t + x0) ** 2)
                terms['loss'] = mean_flat(final_loss)
                terms['final_loss'] = final_loss
                terms['t'] = t
        return terms
    ...
    def get_drift(
        ...
        def data_ode(x, t, model, **model_kwargs):
            if "clamp_pred" in model_kwargs:
                clamp_pred = model_kwargs.pop("clamp_pred")  
                model_output = model(x, t, **model_kwargs)
                print("clampwithin", model_output.max().item(), model_output.min().item())
                model_output = model_output.clamp(min=-clamp_pred, max=clamp_pred)
            else:
                model_output = model(x, t, **model_kwargs)

            t_ = t.view(-1, 1, 1, 1)   
            # model_output = (model_output - x)/(1 - t_).clamp_min(self.t_eps)
            model_output = (x - model_output) / t_.clamp_min(self.t_eps)
            return model_output
    ...
    def get_score(
        ...
        if self.model_type == ModelType.NOISE:
            ...
            def _score_from_x0(x, t, model, **kwargs):
                # predict x0
                x0_hat = model(x, t, **kwargs)
                # get alpha_t and sigma_t used in forward noising
                alpha_t = self.path_sampler.compute_alpha_t(path.expand_t_like_x(t, x))[0]
                sigma_t = self.path_sampler.compute_sigma_t(path.expand_t_like_x(t, x))[0]

                return -(x - alpha_t * x0_hat) / (sigma_t ** 2)

            score_fn = _score_from_x0
