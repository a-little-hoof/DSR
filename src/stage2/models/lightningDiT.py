...
from .model_utils import VisionRotaryEmbeddingFast, SwiGLUFFN, RMSNorm, NormAttention, NormAttentionWithBias, LabelEmbedder, get_2d_sincos_pos_embed, GaussianFourierEmbedding, modulate
...
class LightningDiTBlock(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs
        ...
        if attn_with_bias== True:
            print("Using NormAttentionWithBias in LightningDiTBlock")
            self.attn = NormAttentionWithBias(
                hidden_size,
                num_heads=num_heads,
                qkv_bias=True,
                qk_norm=use_qknorm,
            ...
            print("Using NormAttention in LightningDiTBlock")
...
class LightningFinalLayerNoLN(nn.Module):
    """
    The final layer of LightningDiT.
    """
    def __init__(self, hidden_size, patch_size, out_channels, use_rmsnorm=False):
        ...
        print("Warning: final layer init with no ln")
    def forward(self, x, c):
        shift, scale = self.adaLN_modulation(c).chunk(2, dim=1)
...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        if register_conditioning:
            ...
            self.register_pos_embed = nn.Parameter(torch.randn(1, register_len, hidden_size), requires_grad=False)
            self.register_tokens = nn.Parameter(torch.randn(1, register_len, hidden_size), requires_grad=True)
        ...
        self.blocks = nn.ModuleList([
            LightningDiTBlock(hidden_size, 
                     ...
                     use_rmsnorm=use_rmsnorm,
                     wo_shift=wo_shift,
                     attn_with_bias=attn_with_bias,
                     ) for _ in range(depth)
        ])
        if final_no_ln:
            print("Warning: using final layer without ln")
            self.final_layer = LightningFinalLayerNoLN(hidden_size, patch_size, self.out_channels, use_rmsnorm=use_rmsnorm)
        else:
    ...
    def initialize_weights(self):
        ...
        if self.register_conditioning:
            ...
            register_pos_embed = get_2d_sincos_pos_embed(self.register_pos_embed.shape[-1], int(self.register_len ** 0.5))
            self.register_pos_embed.data.copy_(torch.from_numpy(register_pos_embed).float().unsqueeze(0))
    ...
    def forward(self, x, t=None, y=None):
        ...
        for i,block in enumerate(self.blocks):
            ...
            if self.register_conditioning and i == self.register_start:
                ...
                register_embed = (self.register_tokens + self.register_pos_embed).repeat(batch_size, 1, 1) # (N, register_len, D)
            ...
            if self.register_conditioning and (i > self.register_start or i == self.register_start):
                x = block(x, c, feat_rope=self.feat_rope_incontext)
            else:
                x = block(x, c, feat_rope=self.feat_rope)

        # print(f'x shape before removing in-context tokens: {x.shape}')
        x = x[:, self.register_len:] if self.register_conditioning else x  # remove the in-context tokens
