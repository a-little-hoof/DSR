...
class DiTwDDTHead(nn.Module):
    def __init__(
            ...
            register_conditioning: bool = False,
            register_len: int = 0,
        ...
        self.register_conditioning = register_conditioning
        if register_conditioning:
            print(f'Using in-context conditioning with register length {register_len} starting from block {register_start}')
            self.enc_register_pos_embed = nn.Parameter(torch.randn(1, register_len, hidden_size[0]), requires_grad=False)
            self.dec_register_pos_embed = nn.Parameter(torch.randn(1, register_len, hidden_size[1]), requires_grad=False)
            self.enc_register_tokens = nn.Parameter(torch.randn(1, register_len, hidden_size[0]), requires_grad=True)
            self.dec_register_tokens = nn.Parameter(torch.randn(1, register_len, hidden_size[1]), requires_grad=True)
        ...
        if self.use_rope:
            ...
            if register_conditioning:
                self.enc_feat_rope_incontext = VisionRotaryEmbeddingFast(
                    dim=enc_half_head_dim,
                    pt_seq_len=hw_seq_len,
                    num_cls_token=self.register_len
                )
            ...
            if register_conditioning:
                self.dec_feat_rope_incontext = VisionRotaryEmbeddingFast(
                    dim=dec_half_head_dim,
                    pt_seq_len=hw_seq_len,
                    num_cls_token=self.register_len
                )
    ...
    def initialize_weights(self, xavier_uniform_init: bool = False):
        ...
        if self.register_conditioning:
            print('For register tokens, we are initializing register_pos_embed with fixed sin-cos embedding.')
            enc_register_pos_embed = get_2d_sincos_pos_embed(self.enc_register_pos_embed.shape[-1], int(self.register_len ** 0.5))
            self.enc_register_pos_embed.data.copy_(torch.from_numpy(enc_register_pos_embed).float().unsqueeze(0))
            dec_register_pos_embed = get_2d_sincos_pos_embed(self.dec_register_pos_embed.shape[-1], int(self.register_len ** 0.5))
            self.dec_register_pos_embed.data.copy_(torch.from_numpy(dec_register_pos_embed).float().unsqueeze(0))
    ...
    def forward(self, x, t, y, s=None, mask=None):
        ...
        if s is None:
            ...
            for i in range(self.num_encoder_blocks):
                if self.register_conditioning and i == self.register_start:
                    batch_size = s.shape[0]
                    register_embed = (self.enc_register_tokens + self.enc_register_pos_embed).repeat(batch_size, 1, 1) # (N, register_len, D)
                    s = torch.cat([register_embed, s], dim=1) # (N, T + register_len, D)
                if self.register_conditioning and (i > self.register_start or i == self.register_start):
                    s = self.blocks[i](s, c, feat_rope=self.enc_feat_rope_incontext)
                else:
                    s = self.blocks[i](s, c, feat_rope=self.enc_feat_rope)
            # s = s[:, self.register_len:] if self.register_conditioning else s
            # broadcast t to s
        ...
        if self.register_conditioning:
            ...
            register_embed = (self.dec_register_tokens + self.dec_register_pos_embed).repeat(batch_size, 1, 1) # (N, register_len, D)
            x = torch.cat([register_embed, x], dim=1) # (N, T + register_len, D)
        # print('WARNING: TEST SHAPE', x.shape, s.shape)
        for i in range(self.num_encoder_blocks, self.num_blocks):
            if self.register_conditioning:
                x = self.blocks[i](x, s, feat_rope=self.dec_feat_rope_incontext)
            else:
                x = self.blocks[i](x, s, feat_rope=self.dec_feat_rope)
        x = x[:, self.register_len:] if self.register_conditioning else x
        s = s[:, self.register_len:] if self.register_conditioning else s
