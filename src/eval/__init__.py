...
def compute_generation_metrics(
    ...
    fid = calculate_rfid(rec_arr, ref_arr, batch_size, device_str) 
    return {
        'fid': fid
    }
@torch.no_grad()
def evaluate_generation_distributed(
    ...
    if rank == 0:
        ...
        ref_stats = np.load(ref_npz_path)["arr_0"]
        print(f"[Eval] Loaded reference NPZ from {ref_npz_path}")
        print(f"[Eval] Loaded reference NPZ from {ref_npz_path}, shape: {ref_stats.shape}")
